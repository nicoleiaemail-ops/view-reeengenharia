...
function ResultCard({ icon, target, prefix, suffix, label, sublabel, colorClass, delay }: {
  ...
  return (
    <div className={`scroll-reveal rounded flex items-center gap-5 p-7 relative overflow-hidden transition-all border ${colorClass}`} style={{ transitionDelay: delay }}>
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="text-[.75rem] text-muted-foreground leading-relaxed">
          {label}
          {sublabel && <span className="block text-[.6rem] opacity-50 mt-0.5">{sublabel}</span>}
        </div>
      </div>
    </div>
  );
